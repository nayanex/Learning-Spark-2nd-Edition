...
Underneath the Structured Streaming model, the Spark SQL core engine handles all aspects of fault tolerance and late-data semantics, allowing developers to focus on writing streaming applications with relative ease. Furthermore, Spark 2.x and Spark 3.0 extended the range of streaming data sources to include Apache Kafka, Kinesis, and HDFS-based or cloud storage.
...
Run it like this, considering the structure of this directory:

```bash
 python src/scripts/chapter_2/mnmcount.py "data/raw/chapter_2/mnm_dataset.csv"
```

There is an important technical distinction to make about what happens behind the scenes when you run it this way versus the "official" Spark way.

Here is the breakdown of what happens when you use `python` vs. `spark-submit`.

### 1. Running with `python` (What you typed)
```bash
python src/scripts/chapter_2/mnmcount.py "data/raw/chapter_2/mnm_dataset.csv"
```
If you have `pyspark` installed in your active Python environment (like the `uv` environment), this **will work**. 

* **How it works:** Your computer starts a standard Python process. When Python reaches the line in your script that creates the `SparkSession`, PySpark secretly launches a local Java Virtual Machine (JVM) in the background to run Spark locally on your machine.
* **The Use Case:** This is excellent for local development, quick testing, and debugging on your laptop. 

---

### 2. Running with `spark-submit` (The Production Way)
```bash
spark-submit src/scripts/chapter_2/mnmcount.py "data/raw/chapter_2/mnm_dataset.csv"
```
If you are talking about submitting a "Spark Job" in a production environment (like the DoorDash or Adyen scenarios), `spark-submit` is the industry standard command.

* **How it works:** It reverses the startup order. `spark-submit` starts the Spark JVM *first*, configures the cluster resources (memory, cores, connecting to Kubernetes or YARN), and *then* it launches your Python script inside that distributed environment.
* **The Use Case:** This is required when you want to run your code on a cluster of multiple machines, or when you need to pass specific Spark configurations from the command line (like giving the job more RAM).

### Summary
You can safely use `python script.py` while you are learning and working through Chapter 2 on your laptop. However, once you move your code to a real server or cluster, you will switch to using `spark-submit`.

> The preceding code (`mnmcount.py`) uses the DataFrame API, which reads like high-level DSL queries. Note the clarity and simplicity with which you can instruct Spark **what to do**, not **how to do it**, unlike with the **RDD API**. Cool stuff!
